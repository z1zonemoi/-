...
45. 2022.09.18 오늘은 마지막 강의인 스택, 큐, 딕셔너리에 대해 배웠다. 데이터 구조의 종류들이며, 스택은 가장 먼저 들어온 데이터가 밑에 쌓이는 형태로 맨 윗쪽의 데이터가 최신의 데이터이다. 보통 이메일 같은 경우를 생각해보면 된다. 큐는 스택과 반대의 개념이며 가장 먼저 들어온 데이터가 윗쪽에 쌓이게 된다. 가장 먼저 들어온 값이 가장 먼저 나가게 된다. 딕셔너리는 키와 값이라는 요소로 이뤄져 있으며, 키에 해당하는 값을 저장하고 읽어온다. 생각해보기에서 여태까지 배운 개념들을 기반으로 나만의 새로운 자료 구조를 만들어 볼 수 있을지가 나왔고, 바로 당장은 힘들 것 같지만 앞으로 코드를 많이 다뤄보면서 익숙해진다면 한 번 도전 해보고 싶다. 하루에 하나씩 강의를 듣는게 목표였고, 9.12일 토이 프로젝트로 투두리스트를 만들 때 빼고는 하루에 한 강의씩 듣고 기록을 했다. 이 강의를 기반으로 다른 학습을 할 때 아 맞다 이런 개념이 있었지 정도로만 돼도 내 45일이 헛되지 않을 것 같다. 컴퓨터 과학에 대해 아주아주아주 살짝 가까워진 기분이다ㅋㅋㅋ 앞으로도 파이팅!!